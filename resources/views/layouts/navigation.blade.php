<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @auth
                        <x-nav-link :href="route('pickups.index')" :active="request()->routeIs('pickups.*')">
                    ...
                    @auth
                        @if(auth()->user()->isAdmin())
                            ...
                            <x-nav-link :href="route('admin.pickups.index')" :active="request()->routeIs('admin.pickups.*')">
                                {{ __('Recolecciones') }}
                            </x-nav-link>
                        @endif
                    @endauth
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @auth
                <x-responsive-nav-link :href="route('pickups.index')" :active="request()->routeIs('pickups.*')">
                    {{ __('Mis Recolecciones') }}
                </x-responsive-nav-link>
            @endauth

            {{-- Solo Admin (responsive) --}}
            @auth
                @if(auth()->user()->isAdmin())
                    ...
                    <x-responsive-nav-link :href="route('admin.pickups.index')" :active="request()->routeIs('admin.pickups.*')">
                        {{ __('Recolecciones') }}
                    </x-responsive-nav-link>
                @endif
            @endauth
